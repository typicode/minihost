...
function getDaemonPort (cb) {
  npmconf.load(null, function (err, conf) {
    cb(conf.get('minihost:port') || 3000)
  })
}

function getServerURL (name, cb) {
  getDaemonPort(function (port) {
    cb('http://' + name + '.127.0.0.1.xip.io:' + port)
  })
}
...
module.exports = function (_, opts, cb) {
  var name = opts.name || getServerName()
